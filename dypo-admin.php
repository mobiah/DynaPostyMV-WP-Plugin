...
if(DDEBUG) { error_log("dypo-admin"); }
...
function dypo_saveOptions() {
	...
	if(DDEBUG) { error_log("dyna:admin:dypo_saveOptions"); }
...
function dypo_envTester() {
	...
	update_option(DYPO_OPTIONS, $dypo_options);
	if(DDEBUG) { error_log('dypo-admin: dypo_envTester'); }
...
function dypo_addReporting() {
	...
	if ( !wp_next_scheduled(DYPO_REPORTING_ACTION) ) {
		wp_schedule_event( time(), DYPO_REPORTING_FREQ, DYPO_REPORTING_ACTION ); // hourly, daily and twicedaily
	} 
}
